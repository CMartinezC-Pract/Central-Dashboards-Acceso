...
public class CreateModel : PageModel
{
    ...
    public CreateModel(IIncidenciaService svc, IDashboardService dash,
    ...
    [BindProperty] public IncidenciaCreateDto Input { get; set; } = new();
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        if (Imagenes != null && Imagenes.Count > 0)
            await _imagenes.GuardarImagenesAsync("Incidencia", id, uid, Imagenes);

        await _audit.RegistrarAsync(uid, "CrearIncidencia", "Incidencias", id, folio,
            HttpContext.Connection.RemoteIpAddress?.ToString());

        var admins = await _usuarios.ObtenerAdminsAsync();
        foreach (var admin in admins)
            await _notif.NotificarNuevaIncidenciaAsync(admin.UsuarioID, folio, Input.Titulo, id, nombreUsuario);

        TempData["Exito"] = $"Incidencia {folio} registrada correctamente.";
        return RedirectToPage("./Detail", new { id });
    }

    // ── Handler AJAX (desde modal en Ver.cshtml) ───────────────────────────
    // NOTA: el modal no soporta imágenes por ahora (llamada JSON). Se puede
    //       extender a multipart si se requiere desde el modal.
    public async Task<IActionResult> OnPostAjaxAsync([FromBody] IncidenciaCreateDto dto)
    {
        if (dto == null || string.IsNullOrWhiteSpace(dto.Titulo) ||
            string.IsNullOrWhiteSpace(dto.Descripcion) || dto.TipoIncidenciaID == 0)
        ...
        var admins = await _usuarios.ObtenerAdminsAsync();
        foreach (var admin in admins)
            await _notif.NotificarNuevaIncidenciaAsync(admin.UsuarioID, folio, dto.Titulo, id, nombreUsuario);
    ...
    private async Task CargarTiposAsync()
    {
        ...
        var tipos = db.TiposIncidencia.Select(t => new { t.TipoID, t.Nombre }).ToList();
        Tipos = new SelectList(tipos, "TipoID", "Nombre");
    }
    // ─────────────────────────────────────────────────────────────
    // AGREGAR en Pages/Incidencias/Create.cshtml.cs
    // Este handler recibe FormData (con imágenes) desde Ver.cshtml
    // ─────────────────────────────────────────────────────────────

    public async Task<IActionResult> OnPostAjaxFormAsync(
        [FromForm] int DashboardID,
        [FromForm] int TipoIncidenciaID,
        ...
        [FromForm] List<IFormFile>? Imagenes)
    {
        try
        {
            ...
            foreach (var admin in admins.Where(a => a.UsuarioID != uid))
                await _notif.NotificarNuevaIncidenciaAsync(admin.UsuarioID, folio, Titulo, id, nombre);

            await _audit.RegistrarAsync(uid, "CrearIncidencia", "Incidencias", id,
